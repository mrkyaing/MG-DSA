...
# Function to clean special Unicode characters to ASCII equivalents
def clean_text(text):
    replacements = {
        "’": "'",
        "“": '"',
...
# Your questions data: (Question, Options, Answer, Explanation)
questions = [
...
# Clean all texts for PDF compatibility
cleaned_questions = []
for q, opts, ans, expl in questions:
    q_c = clean_text(q)
    opts_c = clean_text(opts)
    ans_c = clean_text(ans)
    expl_c = clean_text(expl)
    cleaned_questions.append((q_c, opts_c, ans_c, expl_c))

# PDF class with custom header
class PDF(FPDF):
    def header(self):
        self.set_font('Arial', 'B', 14)
        self.cell(0, 10, 'AI-900 Microsoft Azure AI Fundamentals - Practice Questions', 0, 1, 'C')
        self.ln(5)
...
# Create PDF and add questions
pdf = PDF()
pdf.add_page()

for q, opts, ans, expl in cleaned_questions:
    pdf.chapter_title(q)
    pdf.chapter_body(f"{opts}\n\nAnswer: {ans}\nExplanation: {expl}")

# Output PDF file
output_path = "AI-900_Practice_Questions.pdf"
